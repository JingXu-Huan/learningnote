...
>
> 读完本章再进入 `00B-Netty从0到1案例式入门`。目标不是背 API，而是能把每一行示例代码放回“谁在做什么、数据往哪里走”的图里。

> 白话翻译：这是一份“Netty 词典”。你可以先把 `Channel` 理解成电话线，把 `Handler` 理解成工位；遇到陌生词时，先看它在一条消息里负责哪一步。
...
上一章：[[00-学习路线与环境准备]]　下一章：[[00B-Netty从0到1案例式入门]]
