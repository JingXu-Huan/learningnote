...
- [[SQL知识]] —— MyBatis `#{}` 在引号中的使用、CASE WHEN、`<if>` / `<set>` 标签
- [[分页查询]] —— 动态 SQL 配合分页查询使用
- [[多表查询]] —— JOIN 查询基础
- [[MySQL索引/索引和索引下推]] —— 动态 SQL 中条件变化可能导致索引失效
- [[技术栈/Java与框架/MybatisPlus/Spring/SpringBoot/数据库事务]] —— 涉及数据修改的动态 SQL 需要事务管理
